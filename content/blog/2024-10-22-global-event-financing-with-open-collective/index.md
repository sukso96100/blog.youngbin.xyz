...
- 인보이스 제출하여 개인 또는 업체에 비용 지불
- 이미 지불한 것에 대한 영수증 등을 제출하여 환급받기

두 방법 모두 Open Collective 웹 인터페이스로 제출하여 처리하는 방식이여서, 비용 처리 요청 제출 하기도 편하고, 같이 제출된 각종 거래 증빙 관리도 편해서 좋았던 것 같다. 나 같은 경우는 직접 비용 처리 요청 제출을 하는 경우는 잘 없었고, 오히려 올라온 요청을 승인하는 경우가 대부분 이였는데, 이런 요청을 검토하고 승인하기도 편하게 잘 되어 있어서 유용하게 잘 사용 하였다.

![](./pay-request-review.png)

지불 혹은 환급 요청이 들어오면 위 사진처럼 확인이 가능하고, 검토 후 승인을 하면 Open Source Collective 측에서 매주 화요일과 목요일에 일괄적으로 검토 후 지불 처리를 해 준다. 그리고 이에 대한 업데이트도 Open Collective 에서 아래 사진처럼 바로 확인이 가능하다.

![](./payment-update.png)

약간 불편한 부분이 있었다면, 인도에서 행사를 개최 하다 보니 당연하게도 인도 현지 업체에 지불 할 일이 많았는데, 우선 Open Source Collective 에서 국제송금에 쓰는 Wise.com 은 인도 지원이 없었어서 은행 송금을 이용해야 했었다. 그러다 보니 수수료가 좀 더 많이 나가고, 송금 완료 확인도 좀 더 시간이 소요되었다. 인도 현지 업체들은 또 해외송금을 잘 안해봐서 그런 것인지는 모르겠지만, 비용지불을 위해 Swift 송금 정보를 받아야 하는데 로컬 팀에서 제대로 된 Swift 송금 정보를 받는데 어려움이 있는 경우도 자주 있어 살짝 머리가 아픈 경우가 종종 있었다.

## 결산하기
UbuCon Asia 의 경우, "UbuCon Asia Committee" Collective 아래에 "UbuCon Asia 2024" 이벤트를 만들고, 이를 이용해서 자금 관리를 하였다. 덕분에 행사 관련된 것만 모아서 보고서를 바로 확인이 가능했다. 대시보드의 Report 기능으로 기간별 보고서를 확인 할 수가 있다. 비목별로 자세하게 나오지는 않지만, 대략적으로 얼마나 많이 돈이 들어오가 나갔는지 정도는 한눈에 확인이 가능했다.
